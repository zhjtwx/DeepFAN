...
class PatchED3D(BaseModule):
    def __init__(
        ...
        super(PatchED3D, self).__init__(init_cfg=init_cfg)
        assert len(kernel_size) == 3
        assert len(stride) == 3
        assert len(dilation) == 3
        assert padding in ('corner', 'same')
        ...
        self.downsample_layer = build_conv_layer(
            dict(type=conv_type),
            in_channels=in_channels,
            out_channels=in_channels,
            kernel_size=kernel_size,
        ...
        if input_size:
            ...
            w_out = (input_size[2] + 2 * padding[2] - dilation[2] *
                     (kernel_size[2] - 1) - 1) // stride[2] + 1
            self.init_out_size = (d_out // 2, h_out // 2, w_out // 2)
        else:
            self.init_input_size = None
            self.init_out_size = None
    ...
    def forward(self, x):
        ...
        out_size = (x.shape[2], x.shape[3], x.shape[4])
        x = x.flatten().view(-1, self.patch_num, self.embed_dims)
        if self.norm is not None:
            x = self.norm(x)
        return x, out_size